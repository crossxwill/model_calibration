...
```{r}
industry_model <- glm(default_event ~ credit_score, data=industry_df, family=binomial)
...
train_company_df$industry_linear_risk <- predict(industry_model, newdata=train_company_df, type="link")
...
```{r}
calibrated_model <- glm(default_event ~ industry_linear_risk, data=train_company_df, family=binomial)
...
```{r}
test_company_df$industry_linear_risk <- predict(industry_model, newdata = test_company_df, type="link")
```
...
```{r}
test_company_df$pred_calibrated_default <- predict(calibrated_model, newdata=test_company_df, type="response")
```
